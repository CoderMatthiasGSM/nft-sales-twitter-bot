...
async function monitorContractWOMEN() {
  ...
  contract.on('Transfer', async (...params) => {
    ...
    // if more than one asset sold, link directly to etherscan tx, otherwise the marketplace item
    if (tokens.length > 1) {
      tweet(`Many valiant Women of Aradena have joined a new army for a legendary amount of ${totalPrice} ${currency.name} thanks to Sir ${market.name}📯 https://etherscan.io/tx/${transactionHash}`);
        } else {
      tweet(`Woman of Aradena #${tokens[0]} has joined a new army for ${totalPrice} ${currency.name} (${market.name}). Aradena welcomes you ⚔️🍻! #Tcg #NFT #Gaming https://rarible.com/token/${process.env.CONTRACT_ADDRESS}:${tokens[0]}`);
    }
  });
}
